...
export default function UploadBanner() {
  const { status, progress, retry } = useUpload();
  ...
  return (
    <div
      className={cn(
        ...
        isUploading && "bg-card border-t border-border text-foreground"
      )}
      onClick={isError ? retry : undefined}
    >
      {isUploading && <Loader2 className="h-4 w-4 animate-spin flex-shrink-0 text-primary" />}
